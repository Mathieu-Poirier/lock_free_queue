...
TEST(QueueBuilderTest, PassInNullPtrToBuildFilePathThrows) {
    ...
    EXPECT_THROW(
        BuildFilePath(queueBuilder, nullptr, BufferSize(std::size_t{1})),
...
TEST(QueueBuilderTest, EmptyPathWithLengthOneDoesNotThrow) {
    ...
    EXPECT_NO_THROW(
        BuildFilePath(queueBuilder, "", BufferSize(std::size_t{1}))
    );
...
TEST(QueueBuilderTest, NoNullTerminatorThrows) {
    ...
    EXPECT_THROW(
        BuildFilePath(queueBuilder, buf, BufferSize(std::size_t{3})),
        std::invalid_argument
    );
...
TEST(QueueBuilderTest, PathLengthSmallerThanCStringLengthThrows) {
    ...
    EXPECT_THROW(
        BuildFilePath(queueBuilder, str_buf, BufferSize(std::size_t{5})),
        std::invalid_argument
    );
...
TEST(QueueBuilderTest, PathLengthLargerThanCStringLengthThrows) {
    ...
    EXPECT_THROW(
        BuildFilePath(queueBuilder, str_buf, BufferSize(std::size_t{7})),
        std::invalid_argument
    );
...
TEST(QueueBuilderTest, ValidBuildSizeDoesNotThrow) {
    ...
    EXPECT_NO_THROW(
        BuildSize(queueBuilder, BufferSize(std::size_t{8}))
    );
